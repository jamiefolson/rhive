...
public class GenericUDFArraySum extends GenericUDFArrayReduce {
    ...
    protected Object evaluateElement(Object current, Object next)
        ...
        switch (elementOI.getPrimitiveCategory()) {
          case INT: sum = (((IntObjectInspector)elementOI).get(next) +
                                         ((IntObjectInspector)elementOI).get(current));
                                     break;
          case FLOAT: sum = (((FloatObjectInspector)elementOI).get(next) +
                                         ((FloatObjectInspector)elementOI).get(current));
                                     break;
          case DOUBLE: sum = (((DoubleObjectInspector)elementOI).get(next) +
                                         ((DoubleObjectInspector)elementOI).get(current));
                                    break;
          case LONG: sum = (((LongObjectInspector)elementOI).get(next) +
                                         ((LongObjectInspector)elementOI).get(current));
                                      break;
          case SHORT: sum = (((ShortObjectInspector)elementOI).get(next) +
